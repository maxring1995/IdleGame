 ...
 * Run with: npx ts-node scripts/test-quest-integration.ts
 */
...
async function testQuestTracking() {
  ...
  if (existingQuest) {
    ...
    // If completed or failed, reset it
    if (existingQuest.status !== 'active') {
      console.log('   Resetting quest to active...')
      await supabase
        .from('quests')
        .update({
          status: 'active',
  ...
  if (finalQuest) {
    const finalProgress = finalQuest.progress as any
